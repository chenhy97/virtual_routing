...
def sender(cmd,route):
    command = cmd
    A = threading.Thread(target=send_thread,args=(IP_A,PORT_A,command,route,40000))####记得新建一个类叫做route哦
    B = threading.Thread(target=send_thread, args=(IP_B, PORT_B, command, route, 41000))
    C = threading.Thread(target=send_thread,args=(IP_C,PORT_C,command,route,42000))####记得新建一个类叫做route哦
    A.start()
    B.start()
    C.start()
