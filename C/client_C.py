...
def sender(cmd,route):
    command = cmd
    A = threading.Thread(target=send_thread,args=(IP_A,PORT_A,command,route,30000))####记得新建一个类叫做route哦
    D = threading.Thread(target=send_thread,args=(IP_D,PORT_D,command,route,31000))####记得新建一个类叫做route哦
    A.start()
    D.start()
